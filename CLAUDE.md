...
`capture.question_frame_policy` is `latest` or `immediate`. `latest` attaches the most recent frame from the current effective Watch span. `immediate` captures exactly one extra frame, sends the same object to both agents (and to the buffered Observer's active batch), and does not move the periodic deadline. Neither policy may reuse a frame after Watch stops or the Observer stops accepting frames.
